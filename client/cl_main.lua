...
RegisterNetEvent('qb-rental:client:LicenseCheck', function(data)
    ...
    if license == "driver" then
        QBCore.Functions.TriggerCallback("qb-rentals:server:getDriverLicenseStatus", function(hasLicense)
            if not hasLicense then
                return QBCore.Functions.Notify(Lang:t("error.no_driver_license"), "error", 4500)
            end
    ...
    elseif license == "pilot" then
        QBCore.Functions.TriggerCallback("qb-rentals:server:getPilotLicenseStatus", function(hasLicense)
            if not hasLicense then
                return QBCore.Functions.Notify(Lang:t("error.no_pilot_license"), "error", 4500)
            end
            TriggerEvent('qb-rental:client:openMenu', data)
        end)
    end
...
RegisterNetEvent('qb-rental:client:openMenu', function(data)
    ...
    if menu == "vehicle" then
        for k = 1, #Config.Vehicles.land do
            ...
            local name = veh and ('%s %s'):format(veh.brand, veh.name) or
                Config.Vehicles.land[k].model:sub(1, 1):upper() .. Config.Vehicles.land[k].model:sub(2)
            vehMenu[#vehMenu + 1] = {
                id = k + 1,
                header = name,
    ...
    elseif menu == "aircraft" then
        for k = 1, #Config.Vehicles.air do
            ...
            local name = veh and ('%s'):format(veh.name) or
                Config.Vehicles.air[k].model:sub(1, 1):upper() .. Config.Vehicles.air[k].model:sub(2)
            vehMenu[#vehMenu + 1] = {
                id = k + 1,
                header = name,
    ...
    elseif menu == "boat" then
        for k = 1, #Config.Vehicles.sea do
            ...
            local name = veh and ('%s %s'):format(veh.brand, veh.name) or
                Config.Vehicles.sea[k].model:sub(1, 1):upper() .. Config.Vehicles.sea[k].model:sub(2)
            vehMenu[#vehMenu + 1] = {
                id = k + 1,
                header = name,
...
local CreateNPC = function()
    -- Vehicle Rentals
    created_ped = CreatePed(5, Config.Locations.vehicle.pedhash, Config.Locations.vehicle.coords.x,
        Config.Locations.vehicle.coords.y, Config.Locations.vehicle.coords.z, Config.Locations.vehicle.coords.w, false,
        true)
    ...
    -- Aircraft Rentals
    created_ped = CreatePed(5, Config.Locations.aircraft.pedhash, Config.Locations.aircraft.coords.x,
        Config.Locations.aircraft.coords.y, Config.Locations.aircraft.coords.z, Config.Locations.aircraft.coords.w, false,
        true)
    FreezeEntityPosition(created_ped, true)
    SetEntityInvincible(created_ped, true)
    SetBlockingOfNonTemporaryEvents(created_ped, true)
    ...
    -- Boat Rentals
    created_ped = CreatePed(5, Config.Locations.boat.pedhash, Config.Locations.boat.coords.x,
        Config.Locations.boat.coords.y, Config.Locations.boat.coords.z, Config.Locations.boat.coords.w, false, true)
    FreezeEntityPosition(created_ped, true)
    SetEntityInvincible(created_ped, true)
    SetBlockingOfNonTemporaryEvents(created_ped, true)
    TaskStartScenarioInPlace(created_ped, 'WORLD_HUMAN_CLIPBOARD', 0, true)
